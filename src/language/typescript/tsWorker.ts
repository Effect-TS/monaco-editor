...
export class TypeScriptWorker implements ts.LanguageServiceHost, ITypeScriptWorker {
	...
	private static clearFiles(tsDiagnostics: ts.Diagnostic[]): Diagnostic[] {
		return sanitizeDiagnostics(tsDiagnostics);
	}
	...
	async getCompletionsAtPosition(
		fileName: string,
		position: number,
		options: ts.GetCompletionsAtPositionOptions | undefined = undefined
	): Promise<ts.CompletionInfo | undefined> {
		if (fileNameIsLib(fileName)) {
			return undefined;
		}
		return this._languageService.getCompletionsAtPosition(fileName, position, {
			...options,
	...
	async getCompletionEntryDetails(
		...
		position: number,
		entry: string,
		formatOptions: ts.FormatCodeOptions | ts.FormatCodeSettings | undefined = undefined,
		source: string | undefined = undefined,
		preferences: ts.UserPreferences | undefined = undefined,
		data: ts.CompletionEntryData | undefined = undefined
